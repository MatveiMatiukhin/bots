...
def send_welcome(message):
    ...
    if "sendactions" in message.text:
        ...
        bt1 = types.InlineKeyboardButton(
            "ССылка на тг канал", url=f"https://t.me/free_chanel90"
        )
        markup.add(bt1)
...
def send_legend(message):
    bot.send_message(
        message.chat.id,
        "Ходит легенда о бодром боровике, сделавшем этого бота \n@biznesmanil - его имя",
    )
...
@bot.message_handler(
    func=lambda message: message.chat.id in waiting_for_resume
    and waiting_for_resume[message.chat.id]
)
def handle_resume(message):
    ...
    bt9 = types.InlineKeyboardButton(
        "Approve", callback_data=f"approve_{message.chat.id}"
    )
    bt10 = types.InlineKeyboardButton(
        "Reject", callback_data=f"reject_{message.chat.id}"
    )
    markup.add(bt9, bt10)
    ...
    try:
        ...
        if message.content_type == "text":
            bot.send_message(
                developer_chat_id,
                f"Резюме от пользователя (@{message.from_user.username}) на техническое задание №{task_number}:\n{message.text}",
                reply_markup=markup,
            )
        ...
        bot.send_message(
            message.chat.id, f"Произошла ошибка при отправке резюме: {str(e)}"
        )

    del waiting_for_resume[message.chat.id]
    bot.send_message(
        message.chat.id,
        "Ваше резюме было успешно отправлено разработчику! \nЖдите ответа от администратора \nОтвет прийдет в этот чат",
    )


@bot.message_handler(
    func=lambda message: message.chat.id in waiting_for_technical_task
    and waiting_for_technical_task[message.chat.id]
)
def handle_technical_task(message):
    ...
    markup = types.InlineKeyboardMarkup()
    bt20 = types.InlineKeyboardButton(
        "Approve",
        callback_data=f"approve1_{submitter1_id}",
        url=f"https://t.me/NIKITAIvachenko_bot?start=sendactions",
    )
    bt21 = types.InlineKeyboardButton(
        "Reject", callback_data=f"reject1_{submitter1_id}"
    )
    ...
    try:
        msg = bot.send_message(
            chanel_chat_id,
            f"Техническое задание №{technical_task_counter}:\n{message.text}",
            reply_markup=markup,
        )
    ...
    bot.send_message(
        message.chat.id,
        f"Ваше техническое задание было успешно отправлено в тгк под номером {technical_task_counter}! \nЖдите ответа от администратора \nОтвет прийдет в этот чат",
    )


@bot.message_handler(
    func=lambda message: message.chat.id in editing_technical_task
    and editing_technical_task[message.chat.id]
)
    ...
    if chat_id in last_technical_task_message_id:
        ...
        try:
            bot.edit_message_text(
                f"Техническое задание №{task_number}:\n{message.text}",
            ...
            bot.send_message(
                chat_id, f"Произошла ошибка при обновлении сообщения: {str(e)}"
            )
...
def send_photo_0(message):
    ...
    if (
        message.chat.id not in waiting_for_resume
        or not waiting_for_resume[message.chat.id]
    ) and message.chat.id not in authorized_users:
        bot.send_message(
            message.chat.id,
            "Вы не находитесь в режиме ожидания отправки резюме. Пожалуйста, используйте команду /start и выберите действие.",
        )
        return

    elif (
        message.chat.id not in waiting_for_technical_task
        or not waiting_for_technical_task[message.chat.id]
    ) and message.chat.id in authorized_users:
        bot.send_message(
            message.chat.id,
            "Вы не находитесь в режиме ожидания отправки технического задания. Пожалуйста, используйте команду /start и выберите соответствующее действие",
        )
        return
        ...
        try:
            ...
            bot.send_photo(
                developer_chat_id,
                photo_id,
                caption=f"Резюме (фото) на техническое задание №{task_number} от пользователя (@{message.from_user.username})",
            )
            bot.send_message(
                message.chat.id, "Ваше фото было успешно отправлено разработчику."
            )

        except Exception as e:
            if photo_id is None:
                bot.send_message(
                    message.chat.id, "Не удалось получить идентификатор фото."
                )

            else:
                bot.send_message(
                    message.chat.id, f"Произошла ошибка при отправке фото: {str(e)}"
                )
        ...
        try:
            ...
            bot.send_photo(
                developer_chat_id,
                photo_id,
                caption=f"Техническое задание (фото) №{task_number}",
            )
            bot.send_message(
                message.chat.id, "Ваше фото было успешно отправлено в тгк."
            )
...
def send_document_0(message):
    ...
    bot.send_document(
        ...
        caption=f"Резюме (документ) на техническое задание №{task_number} от пользователя (@{message.from_user.username})",
    )

    bot.send_message(
        message.chat.id, "Ваш документ был успешно отправлен разработчику."
    )
...
def callback_message(callback):
    ...
    if callback.data == "customer":
        ...
        bot.send_message(
            chat_id,
            "Пожалуйста, отправьте ваше резюме. Если есть какие-либо файлы, отправьте их вместе с текстом отдельным сообщением.",
        )
        ...
        bot.send_message(
            chat_id,
            "Пожалуйста, отправьте ваше техническое задание. Если есть какие-либо файлы, отправьте их вместе с текстом отдельным сообщением.",
        )
        ...
        if submitter1_id:
            markup = types.InlineKeyboardMarkup()
            bot_link_button = types.InlineKeyboardButton(
                "Перейти к боту",
                url=f"https://t.me/NIKITAIvachenko_bot?start=sendactions",
            )
        ...
        if chat_id in last_technical_task_message_id:
            ...
            try:
                ...
                bot.send_message(
                    chat_id, "Последнее техническое задание было успешно удалено."
                )
                del last_technical_task_message_id[chat_id]

            except Exception as e:
                bot.send_message(
                    chat_id, f"Произошла ошибка при удалении сообщения: {str(e)}"
                )
        ...
        bot.send_message(
            chat_id, "Введите новый текст для последнего технического задания:"
        )
...
def send_actions_menu2(chat_id):
    ...
    bt6 = types.InlineKeyboardButton(
        "Добавить техническое задание", callback_data="add_technical_task"
    )
    bt7 = types.InlineKeyboardButton(
        "Изменить техническое задание", callback_data="edit_technical_task"
    )
    bt8 = types.InlineKeyboardButton(
        "Удалить техническое задание", callback_data="delete_technical_task"
    )
    markup.add(bt6, bt7)
    markup.add(bt8)
    bot.send_message(chat_id, "Выберите предпочитаемое действие:", reply_markup=markup)
...
def resume_approved(submitter_id):
    ...
    bot.send_message(
        submitter_id,
        "Ваше резюме было принято, можете приступать к выполнению заказа.",
        reply_markup=markup,
    )
